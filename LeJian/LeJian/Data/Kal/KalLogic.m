...
- (id)init
{
  if ((self = [super init])) {
    monthAndYearFormatter = [[NSDateFormatter alloc] init];
      //TODO:修改月份
      [monthAndYearFormatter setDateFormat:@"M月份　yyyy"];//NSLocalizedString(@"yyyyYearMMMonth", @"")];
    [monthAndYearFormatter setLocale:[[[NSLocale alloc] initWithLocaleIdentifier:
                                       [[[NSBundle mainBundle] infoDictionary] objectForKey:(id)kCFBundleDevelopmentRegionKey]] autorelease]]; 
...
- (NSArray *)calculateDaysInFirstWeekOfFollowingMonth
{
  ...
  NSUInteger numDays = [self.baseDate cc_numberOfDaysInMonth];
  NSUInteger numPreviousPartialDays = [self numberOfDaysInPreviousPartialWeek];
    numPartialDays = 42 - (numDays + numPreviousPartialDays);
...
- (void)recalculateVisibleDays
{
  ...
  //change the range  by:gongxuehan
  if ([self.daysInSelectedMonth count] > 0)
  {
